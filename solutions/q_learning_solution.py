...
class Q_Agent():
    ...
    def predict(self, state: np.ndarray) -> int:
        ...
        if np.random.uniform(0,1) < self.epsilon: 
            # same as env.action_space.sample()
    ...
    def update_table(self, state: np.ndarray, next_state: np.ndarray, action: int, reward: float):
        ...
        best_next_q_value = np.max(self.qtable[next_state, :])
        self.qtable[state, action] = q_value + self.alpha * (reward + self.gamma * best_next_q_value - q_value)
